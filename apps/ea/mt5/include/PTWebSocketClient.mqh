//+------------------------------------------------------------------+
//|                                        PTWebSocketClient.mqh      |
//|              PineTunnel WebSocket Client - MT5 Wrapper              |
//|                   Phase 2: Real-time signal delivery via WebSocket   |
//|                                                                      |
//| This file wraps the PTWebSocket.dll (x64) for use in MT5 EAs.      |
//| MT4 uses PTWebSocketClient_MT4.mqh with PTWebSocket32.dll (x86).   |
//|                                                                      |
//| Platform: MT5 only (uses long handle type)                           |
//| DLL: PTWebSocket.dll (x64)                                           |
//|                                                                      |
//| *** PARITY: All constants, state values, reconnect logic, heartbeat  |
//| defaults, and error handling MUST be identical to MT4 version. ***   |
//| Only the handle type (long vs int) and DLL filename differ.          |
//+------------------------------------------------------------------+
...
#ifndef PT_WEBSOCKET_CLIENT_MQH
#define PT_WEBSOCKET_CLIENT_MQH

//+------------------------------------------------------------------+
//| Constants - MUST match asyncwebsocketclient.h and MT4 version exactly|
//| These values are IDENTICAL to PTWebSocketClient_MT4.mqh.            |
//| Do NOT change these without updating the MT4 version.               |
//+------------------------------------------------------------------+
...
// Default configuration - MUST match MT4 exactly
...
//| DLL uses DWORD (4 bytes) for each field. MQL5 long is 8 bytes,     |
//| so we use int (4 bytes) to match the DLL struct layout exactly.     |
//+------------------------------------------------------------------+
struct PTWS_ConnectionStats
{
   ...
   int    terminal_lag_ms;     // Milliseconds since last Poll() call (EA responsiveness)
};
...
//+------------------------------------------------------------------+
//| DLL Imports - MT5 uses long handle type and PTWebSocket.dll          |
//| Function signatures are IDENTICAL to MT4 except handle type (long).  |
//+------------------------------------------------------------------+
#import "PTWebSocket.dll"
    long   PTWS_ConnectAuth(string url, int port, int secure, string ws_param, string auth_key);
   void   PTWS_Disconnect(long handle);
   void   PTWS_Reset(long handle);
   int    PTWS_Send(long handle, uchar &data[], int length);
   int    PTWS_Read(long handle, uchar &buffer[], int buffer_size, int &bytes_read, int &buffer_type);
   int    PTWS_Poll(long handle);
   int    PTWS_Readable(long handle);
   int    PTWS_Status(long handle);
   int    PTWS_LastError(long handle);
   int    PTWS_LastCallback(long handle);
    void   PTWS_SetLogLevel(int level);
   int    PTWS_GetLogCount();
   int    PTWS_GetLogEntry(int index, uchar &buffer[], int buffer_size);
   int    PTWS_GetStats(long handle, PTWS_ConnectionStats &stats);
   int    PTWS_GetStatsV2(long handle, PTWS_ConnectionStatsV2 &stats);
   int    PTWS_GetQueueDepth(long handle);
    int    PTWS_SetNotifyWindow(long handle, long wnd, int msg);
   int    PTWS_GetDllVersion(long handle, uchar &buffer[], int buffer_size);
    void   PTWS_RecordPingSent(long handle);
   ...
   int    PTWS_GetSystemInfo(long handle, uchar &buffer[], int buffer_size);
...
//+------------------------------------------------------------------+
//| CPTWebSocketClient - MT5 WebSocket client wrapper                   |
//|                                                                      |
...
//|                                                                      |
//| *** PARITY: All logic MUST be identical to MT4 version. ***          |
//| Only differences: long handle (vs int), PTWebSocket.dll (vs .dll)   |
//+------------------------------------------------------------------+
class CPTWebSocketClient
{
private:
   long    m_handle;              // DLL connection handle (long for MT5)
   ...
   // Prevents trivial extraction from decompiled .ex5 files.
...
public:
  ...
  ~CPTWebSocketClient();
   ...
   int  GetLastError();
   int  GetLastCallback();
   long GetHandle();               // Get current DLL handle (for diagnostics)

   //+------------------------------------------------------------------+
   //| Configuration                                                       |
   //+------------------------------------------------------------------+
   void SetHeartbeat(int interval_seconds);
   bool SetNotifyWindow(long wnd, int msg);  // Set chart HWND for PostMessage notifications
   ...
   , m_rxBufferSize(0)
{
   ...
   // If the DLL is not loaded, MQL5 will set an error and this will fail
   ResetLastError();
...
//| Prevents trivial key extraction from decompiled .ex5 files.           |
//+------------------------------------------------------------------+
...
bool CPTWebSocketClient::Poll()
{
   ...
   if(status == PTWS_CONNECTED || status == PTWS_SENDING || status == PTWS_POLLING)
   {
    ...
    else if(status == PTWS_CLOSING || status == PTWS_CLOSED)
    {
       m_connected = false;
       return false;
    }
...
//+------------------------------------------------------------------+
//| Send account stats via WebSocket                                      |
//| Collects MQL5 account info and sends as JSON heartbeat               |
//+------------------------------------------------------------------+
bool CPTWebSocketClient::SendAccountStats()
{
   ...
   int accCurrencyDigits = (int)AccountInfoInteger(ACCOUNT_CURRENCY_DIGITS);
   bool accFifoClose = (bool)AccountInfoInteger(ACCOUNT_FIFO_CLOSE);
   bool accHedgeAllowed = (bool)AccountInfoInteger(ACCOUNT_HEDGE_ALLOWED);

   // Trade mode enum → string
   ENUM_ACCOUNT_TRADE_MODE tradeMode = (ENUM_ACCOUNT_TRADE_MODE)AccountInfoInteger(ACCOUNT_TRADE_MODE);
   string tradeModeStr = "real";
   if(tradeMode == ACCOUNT_TRADE_MODE_DEMO) tradeModeStr = "demo";
   else if(tradeMode == ACCOUNT_TRADE_MODE_CONTEST) tradeModeStr = "contest";

   // StopOut mode enum → string
   ENUM_ACCOUNT_STOPOUT_MODE soMode = (ENUM_ACCOUNT_STOPOUT_MODE)AccountInfoInteger(ACCOUNT_MARGIN_SO_MODE);
   string soModeStr = (soMode == ACCOUNT_STOPOUT_MODE_PERCENT) ? "percent" : "money";
   ...
   double accMarginInitial = AccountInfoDouble(ACCOUNT_MARGIN_INITIAL);
   double accMarginMaintenance = AccountInfoDouble(ACCOUNT_MARGIN_MAINTENANCE);
   double accAssets = AccountInfoDouble(ACCOUNT_ASSETS);
   double accLiabilities = AccountInfoDouble(ACCOUNT_LIABILITIES);
   double accCommissionBlocked = AccountInfoDouble(ACCOUNT_COMMISSION_BLOCKED);
   ...
   string json = StringFormat(
      ...
      accMarginSoSo,
      accMarginInitial,
      accMarginMaintenance,
      accAssets,
      accLiabilities,
      accCommissionBlocked,
      accCurrencyDigits,
      accFifoClose ? "true" : "false",
      accHedgeAllowed ? "true" : "false",
      PositionsTotal()
   );
...
//| Includes WS latency, terminal lag, and connection stats from DLL    |
//+------------------------------------------------------------------+
bool CPTWebSocketClient::SendHealthTelemetry()
{
   ...
   // Reassemble 64-bit counters from DWORD pairs
   long bytesSent = ((long)stats.bytes_sent_high << 32) | (long)stats.bytes_sent_low;
   long bytesRecv = ((long)stats.bytes_received_high << 32) | (long)stats.bytes_received_low;
   long framesDropped = ((long)stats.frames_dropped_high << 32) | (long)stats.frames_dropped_low;
...
//+------------------------------------------------------------------+
//| Send open positions via WebSocket                                    |
//| Iterates all open positions and sends as JSON array                  |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Send trade history via WebSocket                                    |
//| Sends all deals for the last days_back days (default 7)            |
//+------------------------------------------------------------------+
bool CPTWebSocketClient::SendTradeHistory(int days_back)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      // Skip balance/credit/charge operations - only send actual trades
      ENUM_DEAL_TYPE dealType = (ENUM_DEAL_TYPE)HistoryDealGetInteger(ticket, DEAL_TYPE);
      if(dealType != DEAL_TYPE_BUY && dealType != DEAL_TYPE_SELL)
         continue;

      // Deal entry: in, out, inout, out_by
      ...
      string reasonStr = "client";
      if(reason == DEAL_REASON_EXPERT) reasonStr = "expert";
      ...
      else if(reason == DEAL_REASON_MOBILE) reasonStr = "mobile";
      else if(reason == DEAL_REASON_WEB) reasonStr = "web";
      else if(reason == DEAL_REASON_ROLLOVER) reasonStr = "rollover";
...
long CPTWebSocketClient::GetHandle()
{
   return m_handle;
}
...
//| When a frame arrives, the DLL posts a custom message to this window,|
//| triggering OnChartEvent in the EA for near-instant signal delivery. |
//+------------------------------------------------------------------+
bool CPTWebSocketClient::SetNotifyWindow(long wnd, int msg)
{
   if(!m_initialized || m_handle <= 0)
      return false;
...
//+------------------------------------------------------------------+
//| GetVpsInfo - Detect if running on a VPS                              |
...
//+------------------------------------------------------------------+
//| RunNetworkDiag - Ping a host and check internet connectivity          |
//| host: hostname or IP (empty = "8.8.8.8")                             |
//| timeout_ms: ICMP timeout (0 = 3000ms default)                       |
//| num_pings: number of pings (0 = 3 default)                          |
//| Returns true on success, false on error.                              |
//+------------------------------------------------------------------+
...
//| Returns: 0 on success, 1 if no pending update, negative on error.    |
//+------------------------------------------------------------------+
...
//| Creates a batch script that waits for terminal exit, applies        |
//| pending updates, and relaunches the terminal.                       |
...
//+------------------------------------------------------------------+
//| GetNtpTime - Get UTC time from NTP server + drift from local clock   |
