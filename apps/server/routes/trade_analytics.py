...
# Admin Dashboard HTML endpoint
@admin_router.get("/admin", response_class=HTMLResponse)
async def admin_dashboard(request: Request, user: dict = Depends(get_current_user_optional)):
    """Serve the admin dashboard HTML page (PROTECTED)."""
    # If not authenticated, redirect to login
    if user is None:
        return RedirectResponse(url="/auth/login")
    try:
        # Find the template file
        template_path = Path(__file__).parent.parent.parent / "templates" / "admin_dashboard.html"

        if not template_path.exists():
            # Return inline minimal dashboard if template not found
            return HTMLResponse(content="""
                ...
                <body>
                    <div class="container">
                        <h1>PineTunnel Admin Dashboard</h1>
                        <p class="error">Template file not found. Please ensure admin_dashboard.html is in the templates folder.</p>
                        <p>API endpoints are available:</p>
                        <ul>
                            <li><a href="/api/trades/admin/all">/api/trades/admin/all</a> - All trades</li>
                            <li><a href="/api/trades/admin/stats">/api/trades/admin/stats</a> - Statistics</li>
                            <li><a href="/api/trades/admin/dashboard">/api/trades/admin/dashboard</a> - Dashboard data</li>
                        </ul>
                    </div>
                </body>
                </html>
        ...
        html_content = await asyncio.to_thread(_read_template)
        ...
        return HTMLResponse(
            ...
            + html.escape(str(e))
            + """</p>
                <p><a href="/api/trades/admin/all">View raw API data</a></p>
            </body>
            </html>
        """
        )
...
@admin_router.get("/admin/pro", response_class=HTMLResponse)
async def admin_dashboard_pro(request: Request, user: dict = Depends(get_current_user_optional)):
    ...
    try:
        template_path = (
            Path(__file__).parent.parent.parent / "templates" / "admin_dashboard_pro.html"
        )

        if not template_path.exists():
            return HTMLResponse(content="<h1>Dashboard Pro template not found</h1>")

        def _read_pro_template():
            with open(template_path, "r") as f:
                return f.read()

        html_content = await asyncio.to_thread(_read_pro_template)

        return HTMLResponse(content=html_content)
    except Exception as e:
        logger.error("Error loading dashboard pro: %s", e)
        return HTMLResponse(content=f"<h1>Error: {html.escape(str(e))}</h1>")
